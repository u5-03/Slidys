    ...
    var body: some View {
        HeaderSlide("Step 4: Enable animation according to progress") {
            Item("Prepare an AnimationController and animation progress value", accessory: .number(1)) {
                Item("I used useAnimationController from flutter_hooks for the AnimationController", accessory: .number(1))
            }
            Item("Dynamically determine the drawing range of the Path according to the progress value (0.0-1.0)", accessory: .number(2))
            Item("Execute forward() or repeat() of the AnimationController to run the animation", accessory: .number(3))

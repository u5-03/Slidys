...
public class UnifiedGestureDetector {
    ...
    public func detectGestures(
        ...
        // ジェスチャーデータを作成
        let leftGestureData = leftHandComponent.map { SingleHandGestureData(handTrackingComponent: $0, handKind: .left) }
        let rightGestureData = rightHandComponent.map { SingleHandGestureData(handTrackingComponent: $0, handKind: .right) }
    ...
    public func detectSerialGesture(
        ...
        HandGestureLogger.logDebug("🔍 シリアルジェスチャー検出: \(serialGesture.gestureName) - ステップ \(currentIndex)/\(serialGesture.gestures.count - 1)")
...
public extension UnifiedGestureDetector {
    /// よく使用されるカテゴリの組み合わせ
    static let commonCategories: [GestureCategory] = [
        .pointing,
        .counting,
        .hand
    ]
    static let allCategories: [GestureCategory] = GestureCategory.allCases
    
    /// 高優先度ジェスチャーのみを検出
    func detectHighPriorityGestures(
        from handEntities: [Entity],

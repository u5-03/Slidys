...
public struct PrayerGesture: TwoHandGestureProtocol {
    ...
    public func matches(_ gestureData: HandsGestureData) -> Bool {
        ...
        for finger in fingers {
            ...
            let leftStraight = gestureData.leftHand.handTrackingComponent.isFingerStraight(finger, tolerance: .pi * 70 / 180)
            let rightStraight = gestureData.rightHand.handTrackingComponent.isFingerStraight(finger, tolerance: .pi * 70 / 180)
            
            HandGestureLogger.logDebug("    - \(finger): 左=\(leftStraight ? "伸びている" : "曲がっている"), 右=\(rightStraight ? "伸びている" : "曲がっている")")
        ...
        let leftPalmFacingRight = gestureData.leftHand.handTrackingComponent.isPalmFacingDirection(.right, tolerance: .pi / 4)
        let rightPalmFacingLeft = gestureData.rightHand.handTrackingComponent.isPalmFacingDirection(.left, tolerance: .pi / 4)
        ...
        // 3. 手のひらの中心が近いか確認
        HandGestureLogger.logDebug("  - 手のひら中心間距離: \(palmDistance * 100)cm (閾値: \(palmDistanceThreshold * 100)cm)")
        ...
        HandGestureLogger.logDebug("  - 垂直方向のズレ: \(verticalOffset * 100)cm (閾値: \(verticalOffsetThreshold * 100)cm)")
        ...
        HandGestureLogger.logDebug("  - 手首間距離: \(wristDistance * 100)cm (閾値: \(wristDistanceThreshold * 100)cm)")

...
public struct ArigatouInitialPositionGesture: TwoHandGestureProtocol {
    ...
    public func matches(_ gestureData: HandsGestureData) -> Bool {
        ...
        guard gestureData.leftHand.areAllFingersExtended(),
              gestureData.rightHand.areAllFingersExtended() else {
            return false
        }
        ...
        guard verticalSeparation < 0.10 else {
            HandGestureLogger.logDebug("❌ ArigatouInitial: 垂直差が大きすぎる: \(verticalSeparation)m (必要: < 0.10m)")
            return false
        }
        ...
        HandGestureLogger.logDebug("✅ ArigatouInitial: 検出成功 (手首間: \(wristDistance)m, 垂直差: \(verticalSeparation)m)")
        return true
    }
}
...
public struct ArigatouFinalPositionGesture: TwoHandGestureProtocol {
    ...
    public func matches(_ gestureData: HandsGestureData) -> Bool {
        ...
        guard verticalSeparation >= 0.10 else {
            HandGestureLogger.logDebug("❌ ArigatouFinal: 右手が上にない: \(verticalSeparation)m (必要: >= 0.10m)")
            return false
        }
        ...
        HandGestureLogger.logDebug("✅ ArigatouFinal: 検出成功 (垂直: \(verticalSeparation)m, 手首間: \(wristDistance)m)")
        return true
    }
}

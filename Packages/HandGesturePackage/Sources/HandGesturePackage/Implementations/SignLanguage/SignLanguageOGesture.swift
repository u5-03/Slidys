...
public struct SignLanguageOGesture: SignLanguageProtocol {
    ...
    public func matches(_ gestureData: SingleHandGestureData) -> Bool {
        ...
        guard
            otherFingers
                .allSatisfy({ gestureData.isFingerBentAtLeast($0, minimumLevel: .slightlyBent) })
        else {
            return false
        }
        ...
        guard gestureData.handTrackingComponent.isThumbTouchingFinger(.index, threshold: 0.03)
        else { return false }

...
public struct SignLanguageBGesture: SignLanguageProtocol {
    ...
    public func matches(_ gestureData: SingleHandGestureData) -> Bool {
        ...
        guard gestureData
            .isFingerBentAtLeast(.thumb, minimumLevel: .moderatelyBent) else { 
        ...
        // 4本の指が揃っているかチェック（上向き）
        let allPointingUp = extendedFingers.allSatisfy({ gestureData.isFingerPointing($0, direction: .top) })
        let allPointingForward = extendedFingers.allSatisfy({ gestureData.isFingerPointing($0, direction: .forward) })
        ...
        // 人差し指と中指
        guard gestureData.handTrackingComponent.areFingerTipsTouching(.index, .middle, threshold: touchThreshold) else {
            HandGestureLogger.logDebug("SignLanguageB: 人差し指と中指が離れすぎている")
            return false
        }
        // 中指と薬指
        guard gestureData.handTrackingComponent.areFingerTipsTouching(.middle, .ring, threshold: touchThreshold) else {
            HandGestureLogger.logDebug("SignLanguageB: 中指と薬指が離れすぎている")
            return false
        }
        // 薬指と小指
        guard gestureData.handTrackingComponent.areFingerTipsTouching(.ring, .little, threshold: touchThreshold) else {

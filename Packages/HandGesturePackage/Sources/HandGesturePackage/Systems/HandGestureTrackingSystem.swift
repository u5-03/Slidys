...
public struct HandGestureTrackingSystem: System {
    ...
    /// 統合ジェスチャー検出器
    private let unifiedDetector: UnifiedGestureDetector
    ...
    public init(scene: RealityKit.Scene) {
        ...
        // 統合ジェスチャー検出器を初期化
        self.unifiedDetector = UnifiedGestureDetector(gestures: allGestures)
    }
    
    public func update(context: SceneUpdateContext) {
        let handEntities = context.scene.performQuery(EntityQuery(where: .has(HandTrackingComponent.self)))
...
private extension HandGestureTrackingSystem {
    
    /// 検出されたジェスチャーを処理
    func processDetectedGestures(_ detectedGestures: [DetectedGesture], handEntities: [Entity]) {
        ...
        for detected in detectedGestures {
            ...
            case .twoHand:
                if let distance = detected.metadata["palmDistance"] as? Float {
                    debugMessages.append("両手: \(gesture.gestureName) (距離: \(String(format: "%.2f", distance * 100))cm)")
                } else {
    ...
    /// エラーハンドリング
    func handleDetectionError(_ error: GestureDetectionError) {
    ...
    /// パフォーマンス統計を更新
    func updatePerformanceStats(_ detectedGestures: [DetectedGesture]) {
        let stats = unifiedDetector.searchStats
        Self.sharedGestureInfoStore?.updatePerformanceStats(stats)
        
        // パフォーマンス統計を出力（デバッグ用）
        if detectedGestures.count > 0 {
            HandGestureLogger.logPerformance("📊 検出統計: 検索数=\(stats.searchCount), チェック数=\(stats.gesturesChecked), 平均時間=\(String(format: "%.3f", stats.averageSearchTime * 1000))ms")
        }
    }
}
...
private extension HandGestureTrackingSystem {
    ...
    /// 個別の手の状態を更新
    func updateHandStates(from handEntities: [Entity]) {
        var allFingerDistances: [(finger1: FingerType, finger2: FingerType, distance: Float, hand: HandKind)] = []
        ...
        for entity in handEntities {
            ...
            // 指の状態を更新
            updateFingerStates(handComponent: handComponent, gestureData: gestureData, handKind: handKind)
            
            // 手のひらの向きを更新
            let palmDirection = handComponent.getPalmDirection()
            Self.sharedGestureInfoStore?.updatePalmDirection(direction: palmDirection, forHand: handKind)
            
            // 指同士の距離を計算
            let distances = calculateFingerDistances(handComponent: handComponent, handKind: handKind)
    ...
    /// 指の状態を更新
    func updateFingerStates(handComponent: HandTrackingComponent, gestureData: SingleHandGestureData, handKind: HandKind) {
        ...
        for finger in fingers {
            ...
            Self.sharedGestureInfoStore?.updateFingerState(finger: finger, isExtended: isExtended, forHand: handKind)
            
            // 曲がり具合レベル
            let bendLevel = gestureData.getFingerBendLevel(finger)
            Self.sharedGestureInfoStore?.updateFingerBendLevel(finger: finger, level: bendLevel, forHand: handKind)
            
            // 指の向き
            let direction = gestureData.fingerDirection(for: finger)
            Self.sharedGestureInfoStore?.updateFingerDirection(finger: finger, direction: direction, forHand: handKind)
        }
    }
    
    /// 指同士の距離を計算
    func calculateFingerDistances(handComponent: HandTrackingComponent, handKind: HandKind) -> [(finger1: FingerType, finger2: FingerType, distance: Float, hand: HandKind)] {
        var distances: [(finger1: FingerType, finger2: FingerType, distance: Float, hand: HandKind)] = []
        let fingers: [FingerType] = [.thumb, .index, .middle, .ring, .little]
        
        // 親指と他の指の距離
        for otherFinger in fingers where otherFinger != .thumb {
            if let distance = calculateFingerTipDistance(handComponent: handComponent, finger1: .thumb, finger2: otherFinger) {
                distances.append((finger1: .thumb, finger2: otherFinger, distance: distance, hand: handKind))
            }
        }
        ...
        for pair in adjacentPairs {
            if let distance = calculateFingerTipDistance(handComponent: handComponent, finger1: pair.0, finger2: pair.1) {
                distances.append((finger1: pair.0, finger2: pair.1, distance: distance, hand: handKind))
            }
        }
    ...
    /// 2つの指先間の距離を計算
    func calculateFingerTipDistance(handComponent: HandTrackingComponent, finger1: FingerType, finger2: FingerType) -> Float? {
        ...
        guard let tip1Entity = handComponent.fingers[tipJoint1],
              let tip2Entity = handComponent.fingers[tipJoint2] else {
            return nil
        }
...
private extension HandGestureTrackingSystem {
    ...
    /// 手の色を更新
    func updateHandColors(detectedGestures: [DetectedGesture], handEntities: [Entity]) {
        // まずデフォルト色に戻す
    ...
    /// デフォルト色にリセット
    func resetHandColors(handEntities: [Entity]) {
        for entity in handEntities {
            guard let handComponent = entity.components[HandTrackingComponent.self] else { continue }
    ...
    /// 片手の色を更新
    func updateSingleHandColor(hand: String, handEntities: [Entity]) {
        for entity in handEntities {
            guard let handComponent = entity.components[HandTrackingComponent.self] else { continue }
            let isTargetHand = (hand == "left" && handComponent.chirality == .left) ||
    ...
    /// 両手の色を更新
    func updateBothHandsColor(handEntities: [Entity]) {
        for entity in handEntities {
            guard let handComponent = entity.components[HandTrackingComponent.self] else { continue }
            updateHandColor(handComponent: handComponent, color: .green)
        }
    ...
    /// 手の色を更新
    func updateHandColor(handComponent: HandTrackingComponent, color: Color) {
    ...
    /// デフォルト色を取得
    func getDefaultColor(for chirality: HandAnchor.Chirality) -> Color {
        return chirality == .left ? .orange : .cyan
    }
    
    /// 手話ジェスチャーの処理
    func processSignLanguageGestures(_ detectedGestures: [DetectedGesture], gestureInfoStore: GestureInfoStore) {
        // 1秒間隔のチェック
    ...
    /// シリアルジェスチャーの処理
    func processSerialGestures(handEntities: [Entity]) {
        guard let gestureInfoStore = Self.sharedGestureInfoStore else { return }

...
public enum TwoHandGestureType: CaseIterable {
    ...
    public func matches(handData: HandGestureData) -> Bool {
        ...
        case .rightFistAboveLeftOpenHand:
            ...
            guard abs(handData.rightWristWorldPosition.y - handData.rightForearmWorldPosition.y) < HandGestureType.distanceTolerance * 2,
                  abs(handData.leftWristWorldPosition.y - handData.leftForearmWorldPosition.y) < HandGestureType.distanceTolerance * 2 else {
                return false
            }
            // 右手のひらが左手のひらより上にあるかチェック（y座標が高い）
            return handData.rightPalmWorldPosition.y > handData.leftPalmWorldPosition.y + HandGestureType.distanceTolerance

        case .fingersBent90Degrees:
            // 3. すべての指関節が手のひらに対してほぼ90度の角度（各指の中間角度をチェック）
            // 両手をチェック
            let allJoints = handData.rightJointWorldPositions.merging(handData.leftJointWorldPositions) { (_, new) in new }
            ...
            for (jointName, jointPos) in allJoints {
                ...
                if jointName.isIntermediateJoint {
                    ...
                    guard let parentPos = allJoints[parentJoint],
                          let childPos = allJoints[childJoint] else {
                        continue
                    }
        ...
        case .rightArmVerticalLeftArmHorizontalFingertipsTouch:
            ...
            guard abs(rightWristToForearm.x) < HandGestureType.distanceTolerance * 2,
                  ...
                  abs(rightWristToForearm.z) < HandGestureType.distanceTolerance * 2 else {
                return false
            }
